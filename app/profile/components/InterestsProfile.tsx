    ...
    <div className="min-h-screen px-10 py-4 w-full space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 max-w-5xl mx-auto">
            {interests.map((interest, index) => {
                ...
                return <div key={index} className="flex flex-col items-center gap-3 p-6 rounded-xl border hover:shadow-lg transition-all duration-200 border-crdBorder hover:bg-[#2f0d0d]/70 cursor-pointer bg-[#1a1a1a] group">
                        <div className="w-12 h-12 transition-all duration-200 bg-[#c25c5c]/30 group-hover:bg-[#741e1e] rounded-full flex items-center justify-center">
                            {IconComponent && <IconComponent className="w-6 h-6 text-[#de4d4d]" />}
                        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {Object.entries(lifestyle).map(([key, val])=>{
                ...
                return <div key={key} className="flex items-center gap-4 p-6 bg-[#1a1a1a] rounded-xl border border-crdBorder hover:shadow-lg transition-all hover:bg-[#2f0d0d]/70 duration-200 cursor-pointer group">
                    {IconComponent && (
                                <div className="flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center group-hover:bg-[#741e1e] transition-colors bg-[#c25c5c]/30">
                                    <IconComponent className="w-7 h-7 text-[#de4d4d]" />
                                </div>
                    )}
